...
export async function getDisplayName(): Promise<string> {
  const user = await currentUser();
  const profile = user?.profile as { name?: string; preferred_username?: string; email?: string } | undefined;
  return profile?.name ?? profile?.preferred_username ?? profile?.email ?? "Household Member";
}
