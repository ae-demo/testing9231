...
// Added by this app: src/auth.ts exports these two beyond the standard
// surface, so the mock substitution needs matching mocks or the swap fails to
// compile under `--mode mock`.
export async function getCurrentUserId(): Promise<string | null> {
  return signedOut() ? null : user().profile.sub;
}

export async function getDisplayName(): Promise<string> {
  return signedOut() ? "Household Member" : user().profile.name;
}
